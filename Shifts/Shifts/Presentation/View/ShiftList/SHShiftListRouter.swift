...
class SHShiftListRouter {
    ...
    private unowned var routerController: SHShiftsViewController
...
extension SHShiftListRouter: SHShiftListRouterProtocol {
    func routeToAddShift() {
        let destinationVC = SHAddShiftViewController()
        destinationVC.configurator.set(delegate: self.routerController)
